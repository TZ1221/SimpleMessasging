 ...
 * @version 1.3
 */
public final class ChatterSocketNB {
	 ...
	 *            connect
	 */
	public ChatterSocketNB(String host, int port) {
		hostname = host;
		imPort = port;
	 ...
	 * <p>
	 * Once a ChatterSocketNB has been closed, it is not available for further
	 * networking use (i.e. can't be reconnected or rebound). A new ChatterSocketNB
	 * needs to be created.
	 * 
	 * @throws IOException
	...
	/**
	 * This method will block while it waits to enqueue 1 (or more) chatterMessages
	 * sent from the server.
	 * 
	 * @param chatterMessages
	 *            Queue to which the chatterMessages should be added.
	 */
	protected void enqueueMessages(List<ChatterMessage> chatterMessages) {
		...
		try {
			...
			// Start scanning the buffer for any and all chatterMessages.
			int start = 0;
			...
			while ((start + MIN_MESSAGE_LENGTH) <= charBuffer.limit()) {
				...
				ChatterMessage newMsg = ChatterMessage.makeMessage(handle, sender, message);
				...
				if (newMsg.getType() == ChatterMessage.MessageType.QUIT) {
					quitter = true;
				}

				// Now pass this message on to the system.
				chatterMessages.add(newMsg);
			...
			if (start != 0) {
				// Move any read chatterMessages out of the buffer so that we can add
				// to the end.
				buff.position(start);
	...
	/**
	 * Send a ChatterMessage over the network. This method performs its actions by
	 * printing the given ChatterMessage over the ChatterSocketNB instance with which the
	 * SendIM was instantiated.
	 * 
	 * @param msg
	 *            ChatterMessage to be sent out over the network.
	 */
	protected void print(ChatterMessage msg) {
		if (!isConnected()) {
